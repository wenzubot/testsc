const axios = require('axios'); // Tambahkan ini untuk mengimpor axios
...
let handler = async (m, { conn, text, command }) => {
  ...
  let kemii;
  try {
    kemii = await tiktoks(`${text}`);
  } catch (err) {
    return conn.reply(m.chat, 'Error: ' + err, m);
  }
      ...
      message: {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          header: proto.Message.InteractiveMessage.Header.create({
            ...
            ...(await prepareWAMessageMedia({ video: { url: kemii.no_watermark } }, { upload: conn.waUploadToServer }))
          }),
...
handler.help = ['tiktoksearch','tts']
handler.tags = ['downloader'];

handler.command = /^tiktoksearch|tts$/i;
handler.limit = 4;
